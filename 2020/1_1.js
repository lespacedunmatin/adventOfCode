...
function splitData() {
	data.forEach(entry => {
		if (entry >= halfTarget) {
			overHalfTarget.push(entry)
		} else {
			bellowHalfTarget.push(entry)
		}
	})
}

function sortArrays() {
	overHalfTarget.sort((a,b) => b - a)
	bellowHalfTarget.sort((a,b) => a - b)
}

function trySums() {
	overHalfTarget.every(over => {
		if (result) {
			return false
		}
		
		bellowHalfTarget.every(bellow => {
			let sum = over + bellow;
