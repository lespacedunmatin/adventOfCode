...
function treatData(data) {
	function treatLine(line) {
		...
		return (
			(firstPairStart <= secondPairStart && firstPairStop >= secondPairStop) ||
			(secondPairStart <= firstPairStart && secondPairStop >= firstPairStop)
		) ? 1 : 0
	}
	
	const lines = data.split('\n')
	
	console.log(lines.reduce((acc, line) => acc + treatLine(line), 0)) // expected: 462
}
