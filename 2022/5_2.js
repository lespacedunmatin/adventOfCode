...
function treatData(data) {
	...
	const finalSituation = (() => {
		...
		while (craneInstructions.length > 0) {
			...
			const cratesMoving = stacks[origin - 1].slice(-1 * qty)
			stacks[origin - 1] = stacks[origin - 1].slice(0, stacks[origin - 1].length - qty)
			stacks[destination - 1] = `${stacks[destination - 1]}${cratesMoving}`
		}

		return stacks
	})()

	console.log(finalSituation.reduce((acc, stack) => acc + stack.slice(-1), '')) // expected: VRQWPDSGP
}
